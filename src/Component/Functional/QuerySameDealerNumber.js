    ...
    useEffect(() => {
        ...
        if (!Zreg.test(state)) {
            ...
            if (!((asC2 !== "" && !Zreg.test(asC2)) || (asC3 !== "" && !Zreg.test(asC3)) || (asC4 !== "" && !Zreg.test(asC4)) || (asC5 !== "" && !Zreg.test(asC5)))) {
                let url = new URL(Globalcontext.UrlHeader + "api/WinNumbers/GetSameDealerNumber?lasterPeriod=" + state + "&SameDealerNumberHowManyTimes=" + next + str);
                const PData = async () => {
    ...
    return (
        <LeftNavFlexCol>
           ...
           <LeftNavFlexRow>
                ...
                <LasterPeriodTextField1 variant="outlined" label="連莊幾期 : " helperText={vaildnext} value={next} onChange={(e) => { setnext(e.target.value) }} />
